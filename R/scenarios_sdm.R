...
#' i
#'
#' @importFrom stars st_as_stars read_stars st_xy2sfc st_dimensions
#' @importFrom sf st_crs st_make_grid st_transform st_as_sf st_centroid st_coordinates st_bbox st_make_valid st_is_valid
#' @importFrom raster crop mask coordinates crs
...
scenarios_sdm.RasterStack <- function(x, study_area = NULL, predictors_names = NULL, rescaling = NULL,
  ...
  xb <- x
  x <- stars::st_as_stars(cbind(raster::coordinates(x), as.data.frame(x)))
  ...
  if (!is.null(study_area) & !is.null(rescaling)) {
    ...
    coords <- as.data.frame(sf::st_coordinates(sf::st_centroid(grd)))
    cell_id <- grd$cell_id
  } else {
    resolution <- sf::st_res(x)
...
scenarios_sdm.stars <- function(x, study_area = NULL, vars_study_area = NULL, predictors_names = NULL,
  ...
  if (!is.null(study_area) & is.null(rescaling)) {
    ...
    cellsize <- rescaling$cellsize
    if (is.null(rescaling$epsg)) {
      crs2 <- as.character(sf::st_crs(x))[1]
    } else {
      crs2 <- as.character(sf::st_crs(rescaling$epsg))[1]
    }
    study_area <- sf::st_transform(study_area, crs2)
