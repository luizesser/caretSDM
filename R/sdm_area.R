...
.sdm_area_from_sf_using_stars <- function(x, cell_size = NULL, crs = NULL) {
  bbox <- sf::st_bbox(x)
  n_col <- ((bbox$xmax - bbox$xmin) / cell_size) |>
    unname() |>
    ceiling()
  n_row <- ((bbox$ymax - bbox$ymin) / cell_size) |>
    unname() |>
    ceiling()
  ...
  if (error_collection$isEmpty() & "POLYGON" %in% sf::st_geometry_type(x$grid)) {
    ...
    if (invalid_row > 0) {
      error_collection$push(
        glue::glue(
          "The polygon of the row { invalid_row }",
...
.detect_sdm_area <- function(x, cell_size, crs, gdal, lines_as_sdm_area) {
  ...
  if (invalid_row > 0) {
    error_collection$push(
      glue::glue(
        "The cell size of the polygon of the row { invalid_row }",
  ...
  if (sf::st_crs(x) != sf::st_crs(crs)) {
    ...
    error_collection$push(
      glue::glue(
        "Detected CRS ({ crs_calc }) is different from ",
