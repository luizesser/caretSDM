...
#' @param ensemble_type Character of the type of ensemble to be plotted.
#' @param raster Should the niche be extrapolated to a raster covering all possibe values in the
#' environmental space?
#' @param ... Plotting arguments to pass to ggplot2 function.
#'
...
#' @rdname plot_occurrences
#' @export
plot_predictions <- function(i, spp_name = NULL, scenario = NULL, id = NULL, ...) {
  assert_class_cli(i, "input_sdm")
  assert_subset_cli(spp_name, species_names(i))
  assert_names_cli(names(i), must.include = "predictions")
  return(plot(i$predictions, spp_name, scenario, id, ...))
}

#' @exportS3Method base::plot
plot.predictions <- function(x, spp_name = NULL, scenario = NULL, id = NULL, ...) {
  ens <- "predictions"
...
plot_niche <- function(i, spp_name = NULL, variables_selected = NULL, scenario = NULL, id = NULL,
  ...
  if ("ensembles" %in% names(i)) {
    valid_scen <- i$ensembles$data |> colnames()
  } else {
