...
#' @importFrom sf st_transform st_centroid st_as_sf st_crop st_bbox st_crs
...
add_predictors <- function(sdm_area, pred, variables_selected = NULL) {
  if (!is_sdm_area(sdm_area)) {
    cli::cli_abort("first argument is not of class sdm_area")
  }
...
add_predictors.stars <- function(sdm_area, pred, variables_selected = NULL) {
  ...
  grd <- sdm_area$grid
  grd <- sf::st_transform(grd, crs = sf::st_crs(pred))
  suppressWarnings(grd2 <- pred |>
    ...
    aggregate(grd, mean) |>
    cbind(grd) |>
    dplyr::select(-c("geometry.1")))
  grd2 <- sf::st_transform(grd2, crs = sf::st_crs(sdm_area$grid))
  bbox2 <- sf::st_bbox(grd2)
  var_names <- grd2 |>
    tibble::as_tibble() |>
