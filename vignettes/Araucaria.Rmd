...
occ <- GBIF_data('Araucaria angustifolia', as_df=T)
...
Now both predictors and scenarios should have the same variables names. In any case, you can change the names by using the `set_predictor_names` function.
...
```{r predict_sdm}
i <- predict_sdm(i, th = mean, ensembles = TRUE)
i
```
...
```{r plot_predictions}
plot_predictions(i, ensemble_type = "committee_avg", scenario = "585_10_209")
```
