...
occ <- GBIF_data('Salminus brasiliensis', as_df = T)
...
sa2 <- sdm_area(study_area, cell_size = 25000, crs = 6933, gdal = T)
```
...
sa
#write_gpkg(sa, "output_data", "predictors_data.gpkg")
plot_predictors(sa)
```


### Importar dados da 
An alternative to visualize predictors' data is the use of mapview:

```{r mapview_predictors}
...
sa <- set_predictor_names(sa, c("LENGTH_KM", "DIST_DN_KM", "DIST_UP_KM", "CATCH_SKM", "UPLAND_SKM", "DIS_AV_CMS", "ORD_STRA","bio01","bio02", "bio03", "bio04", "bio05", "bio06", "bio07", "bio12", "bio13", "bio14", "bio15"))
sa
```
...
oc <- occurrences_sdm(occ, crs = 4326) |> #### importar .csv direto da pasta
  join_area(sa)
oc
```
