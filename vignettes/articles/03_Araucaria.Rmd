---
title: "3. caretSDM Workflow for Species Distribution Modeling"
author: '[author name]'
date: "`r Sys.Date()`"
output:
  pdf_document:
  ...
  html_document:
    ...
    code_folding: show
    df_print: paged
vignette: "%\\VignetteIndexEntry{3. caretSDM Workflow for Species Distribution Modeling}
  %\\VignetteEncoding{UTF-8} %\\VignetteEngine{knitr::knitr}\n"
editor_options:
  markdown:
    wrap: 100
---
...
```{r gbif_data1, eval = FALSE}
occ <- GBIF_data(c("Araucaria angustifolia"), as_df = TRUE)
```

To make this process a little more difficult for our package to solve, we can transform the `occ` 
...
trouble passing `data.frame`s to the package. As we already have a `occ` object included in the 
...
To avoid the use of external packages, it is possible to add predictors, study area and scenarios
directly through `caretSDM` package as follows:

```{r caretSDM_only, eval = FALSE}
sa <- sdm_area(
...
We strongly recommend the use of this approach to avoid errors that could arise from other packages.
...
is in occ_crs = 4326, but our records stored in `occ` object is transformed to 6933 (see `?occ` for more
information on the data).
...
will only predict models that passes a given validation threshold. This validation metric is set
using `metric` and `th` arguments. In the following example, metric is set to be "ROC" and `th` is 
equal 0.9. This means that only models with ROC \> 0.9 will be used in predictions and ensembles.
...
of arguments an functioning. For plot_ensembles, we can set some parameters to control what is
