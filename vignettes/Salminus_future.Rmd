...
```{r export_grid}
write_grid(sa, "results/predictors_data.gpkg")
```
...
Or we can export the .csv file with coordinates:

```{r}
write_occurrences(i, path = "results/occurrences.csv")
```
...
A important result is also the validation metrics, which can be assessed using `mean_validation_metrics` function:
...
#### Installing chooseGCM

To install chooseGCM we need devtools to retrieve the package from GitHub in the same way we install caretSDM.
...
#WorldClim_data(path="input_data/WorldClim_data_chooseGCM/", period = "future", ssp = '585', year = '2090', gcm = "all", resolution = 10)
WorldClim_data(path="/Users/luizesser/Documents/GitHub/input_data/WorldClim_data_future_all/", period = "future", ssp = '585', year = '2090', gcm = "all", resolution = 10)
...
```{r}
res <- chooseGCM::compare_gcms(s, var_names=c("bio01", "bio12"), sf::st_transform(study_area, crs=4326), k = 2)
res
```
...
```{r}
i2 <- add_scenarios(i, scen, stationary=c("LENGTH_KM", "DIST_DN_KM", "DIST_UP_KM", "CATCH_SKM", "UPLAND_SKM", "DIS_AV_CMS"))
i2
```

```{r}
i2 <- predict_sdm(i2, th = 0.8)
i2
```

```{r}
plot_predictions(i2, scenario = "ch_ssp585_10_209")
```

```{r}
plot_predictions(i2, scenario = "current")
```

```{r}
gcms_ensembles(i2, gcms=res$suggested_gcms)
```
